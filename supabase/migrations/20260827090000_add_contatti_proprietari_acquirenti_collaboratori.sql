-- Pivot Proprietari/Acquirenti/Collaboratori, Fase 1 (schema): sostituisce `leads`
...
--
-- Design: docs/DECISIONI.md "Pivot Proprietari/Acquirenti/Collaboratori".

-- -----------------------------------------------------------------------------
-- contatti: tabella base condivisa. proprietari/acquirenti/collaboratori si
...
-- -----------------------------------------------------------------------------
-- acquirenti: stessa pipeline stato di leads.stato oggi (Nuovo/Contattato/
...
CREATE TABLE public.acquirenti (
    ...
    deleted_at         timestamptz,
    _version           integer NOT NULL DEFAULT 1,
    CONSTRAINT acquirenti_pkey PRIMARY KEY (id),
    CONSTRAINT acquirenti_id_fkey FOREIGN KEY (id) REFERENCES public.contatti(id) ON DELETE CASCADE,
    CONSTRAINT acquirenti_stato_check CHECK (stato = ANY (ARRAY['Nuovo'::text, 'Contattato'::text, 'Trattativa'::text, 'Chiuso'::text, 'Perso'::text]))
);

CREATE INDEX idx_acquirenti_email ON public.acquirenti USING btree (email);
CREATE INDEX idx_acquirenti_stato ON public.acquirenti USING btree (stato);

ALTER TABLE public.acquirenti ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Consenti accesso completo agli agenti autenticati" ON public.acquirenti
    AS PERMISSIVE FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);
...
-- chiunque non sia né proprietario né acquirente. Tabella nuova, nessun dato
...
-- -----------------------------------------------------------------------------
-- acquirenti_immobili: sostituisce lead_immobili per il lato acquirente.
-- lead_immobili resta intatta finché il frontend non passa a questa tabella
-- (Fase 3/4).
-- -----------------------------------------------------------------------------
CREATE TABLE public.acquirenti_immobili (
    id               uuid NOT NULL DEFAULT gen_random_uuid(),
    ...
    CONSTRAINT acquirenti_immobili_pkey PRIMARY KEY (id),
    CONSTRAINT acquirenti_immobili_acquirente_id_immobile_id_key UNIQUE (acquirente_id, immobile_id),
    CONSTRAINT acquirenti_immobili_acquirente_id_fkey FOREIGN KEY (acquirente_id) REFERENCES public.acquirenti(id) ON DELETE CASCADE,
    CONSTRAINT acquirenti_immobili_immobile_id_fkey FOREIGN KEY (immobile_id) REFERENCES public.immobili(id) ON DELETE CASCADE
);

ALTER TABLE public.acquirenti_immobili ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Consenti accesso completo agli agenti autenticati" ON public.acquirenti_immobili
    AS PERMISSIVE FOR ALL TO authenticated
    USING (true)
    WITH CHECK (true);
...
-- per acquirente) — solo per idempotenza dello script di backfill, da rimuovere
